...
Jde totiž o~stejnou amortizovanou Bayesovskou inferenci, jen o~jednu dimenzi výš a s~binárním cílem.
...
Výsledkem je tzv. \emph{maska}, což je obrázek stejného rozměru jako vstup, ve kterém je každý pixel buď uvnitř oblasti nebo mimo.
Masku si můžeme představit jako černobílou obrys obtaženou přes původní obrázek.
\par

Hlavní rozdíl oproti regresi je ve výstupu.
V~regresi PFN předpovídalo jedno reálné číslo pro jeden vstupní bod.
...
Představme si teď stejnou myšlenku, ale s~funkcí \textit{dvou} proměnných, tedy s~náhodným polem přes celou plochu obrázku.
Hladkost tohoto skrytého pole řídí stejný \emph{lengthscale} $\ell$ jako dřív, malé $\ell$ dává drobné a ostré útvary, velké $\ell$ velké hladké plochy.
...
Zvolíme práh a řekneme, že pixel je uvnitř oblasti právě tehdy, když hodnota pole v~něm práh překročí.
...
Tady opět postupujeme podle standardního postupu.
...
Aposteriorní prediktivní rozdělení (PPD), které bylo v~regresi Gaussovou hustotou, je zde v~každém pixelu Bernoulliho rozdělení, tedy pravděpodobnost, že pixel patří do oblasti.
...
Věta~\ref{theorem:Rozptyl_transformeru} o~mizení rozptylu i neredukovatelný bias z~podmínky lokality (věta~\ref{theorem:lokalizace}) jsou tvrzení o~\emph{attention mechanismu} obecně, a měly by tedy platit i pro segmentaci.
Právě tohoto faktu využijeme.
Nejprve vezmeme hotový, už natrénovaný \emph{in-context} segmenter UniverSeg (zde přidat reference).
...
Když má model málo příkladů, nedokáže z~nich poznat pravidla úlohy, a nejbezpečnější sázkou je pro něj průměr.
Přesně toto chování předpovídá podmínka lokality, predikce daleko od dat zůstává přitažená k~prioru.
...
Tady využijeme hlavní výhodu umělých dat, pravidla jejich generování známe, a proto umíme pro každý obrázek spočítat i přesný posterior.
Tomu budeme stejně jako v~jedné dimenzi říkat \emph{oracle}.
U~reálných segmentačních dat takovou pravdu nikdy nemáme, tady proti ní můžeme přesně změřit, jak věrně ji PFN aproximuje a kde vzniká odchylka, tedy bias, rozptyl, kalibraci a chování mimo trénovací rozdělení.
Zkoumáme tedy, jak dobře PFN aproximuje ideální Bayesovskou inferenci a kde se od ní začíná odchylovat, jen teď na náročnější úloze.
...
Jako model použijeme UniverSeg (zde přidat reference), což je předtrénovaný \emph{in-context} segmenter.
Model necháme zmražený, žádnou jeho váhu neupravujeme.
UniverSeg nabízí interní průměrování přes více kontextových sad.
Voláme ho vždy nad jedním \emph{support setem}, tedy bez toho interního průměrování.
...
Postupujeme přes tři domény, snímky bílých krvinek (WBC-JTSC), jinou akvizici týchž buněk (WBC-CV) a mozkové MRI (OASIS).
...
Za povšimnutí stojí, že poloha křížení nesouvisí s~tím, jak moc se doména liší od tréninku.
Kolaps tedy nezpůsobuje to, že doména je pro model neznámá nebo cizí, ale malý počet ukázek.
...
Tím se přímo potvrzuje Naglerova předpověď, a to i na modelu, který není úplně \emph{PFN-type}.
\par
...
Obrázek~\ref{fig:seg-a-dice} ukazuje, že \emph{Dice} s~rostoucím kontextem roste a poté saturuje, přidávat další kontext se přestane vyplácet zhruba od osmi až šestnácti.
...
Tuto shodu měříme \emph{reliability diagramem}, který proti sobě vynáší predikovanou pravděpodobnost a skutečnou frekvenci popředí, a souhrnným číslem, očekávanou kalibrační chybou (ECE).
Důležité je, že směr případné chyby nepředpokládáme, ale měříme.
\par
...
U~mozku je naopak přehnaně sebejistý, a to skoro nezávisle na velikosti kontextu.
Znaménkový rozdíl mezi jistotou a správností je kladný ve všech doménách a nejsilnější při jednom příkladu, s~rostoucím kontextem klesá k~nule.
Když navíc oddělíme vnitřek objektu od úzkého pásma kolem hranice, ukáže se, že se veškerá nejistota soustředí právě na hranici.
...
Poznamenejme, že tento závěr je v~souladu se vzorem, který jsme viděli i v~jedné dimenzi, model dobře pozná, \textit{kde} je nejistý, ale absolutní míru nejistoty odhaduje hůř.
\par

\begin{figure}[!tb]
    \centering
    \includegraphics[width=\linewidth]{figures/VariantA_universeg/fig_A_03_reliability.png}
    \caption{\emph{Reliability} diagramy pro tři domény (řádky) a vybrané velikosti \emph{support setu} (sloupce). Křivka nad úhlopříčkou znamená podhodnocenou jistotu, pod úhlopříčkou přehnanou. Krvinky (WBC) leží nad úhlopříčkou, mozek (OASIS) pod ní.}
...
Podstatné je, že kolaps řídí právě velikost kontextu, a ne to, jak vzdálená je daná doména.
Navíc napříč doménami zůstává jev kvalitativně stejný.
\par

Současně se ukázala i mez tohoto testu.
Poznamenejme, že UniverSeg nemá pravý prior ani dostupný posterior, proto jsme se museli spolehnout na operační prior masku a nemohli jsme predikci porovnat se skutečnou pravdou.
...
Korelační délku, sílu šumu i podíl popředí (tedy jak velkou část obrázku zabírá oblast zájmu) losujeme pro každou dávku znovu ze širokých intervalů, aby se model musel naučit celou rodinu úloh, ne jednu konkrétní.
...
Model trénujeme od nuly na datech z~našeho prioru, s~pixelovou Bernoulliho hlavou a s~binární křížovou entropií jako kritériem.
...
Torus je jen pomůcka pro tento výpočet a na interpretaci výsledků nemá vliv.
\par
...
Uvnitř trénovacího rozsahu proto rozlišujeme úlohy snadné (\emph{Easy}), střední (\emph{Medium}) a náročné (\emph{Hard}).
K~nim přidáme dva režimy mimo trénovací rozsah, ve kterých je korelační délka kratší (\emph{OOD-short}, členitější pole) nebo delší (\emph{OOD-long}, hladší pole), než jakou model při tréninku viděl.
...
\begin{figure}[p]
    ...
    \caption{Průměrná vzdálenost predikce PFN od \emph{oraclu} jako funkce velikosti kontextu pro pět režimů náročnosti. Uvnitř trénovacího rozsahu (Hard, Medium, Easy) je vzdálenost malá a na velikosti kontextu skoro nezávisí. Mimo rozsah (OOD-short, OOD-long) je větší, členitější pole (OOD-short) model nedožene ani s~velkým kontextem.}
...
Mapy PFN a \emph{oraclu} jsou okometricky téměř k~nerozeznání.
Viditelný rozdíl zůstává jen na tenkých hranicích mezi popředím a pozadím.
\par

\textbf{Diskuze:}

Rozdíl mezi PFN a \emph{oraclem} znázorňuje obrázek~\ref{fig:seg-c-fidelita}, který vynáší jejich průměrnou vzdálenost jako funkci velikosti kontextu pro všech pět režimů.
Plyne z~něj poznatek, který se v~této sekci bude opakovat.
Velikost kontextu skoro nehraje roli, predikce je dostatečně dobrá i s~jediným \emph{support párem}, tedy s~jedinou dvojicí obrázku a masky v~kontextu.
Vysvětlení je přímočaré.
Každý pixel vstupního obrázku je zašuměné pozorování skrytého pole v~jednom bodě.
Obrázek jako celek tedy zachycuje pole ve všech bodech najednou.
Model si proto skryté pole dokáže z~velké části zrekonstruovat už ze samotného obrázku, bez pomoci kontextu.
...
Nejhůř dopadají pole s~kratší korelační délkou, než jakou model viděl při tréninku.
U~nich chyba zůstává velká i s~velkým kontextem.
Naopak hladší pole model dožene, jakmile dostane trochu kontextu.
...
To, že střední hodnota vychází smysluplně ještě nemusí nutně neznamenat, že PFN správně reprezentuje i nejistotu.
...
Dále můžeme pozorovat tenkou oblast nejistoty uprostřed, která odpovídá hranicím.
PFN tedy nejistotu na hranicích zachycuje, nesnaží se ji uměle potlačit.
...
Měříme je tak, že pro pevný vstupní obrázek opakovaně losujeme \emph{support set} a sledujeme, jak se predikce mění.
...
Rozptyl by tedy měl mizet aspoň jako $n^{-1}$.
...
Pravý graf vynáší tento rozdíl, tedy přebytečnou chybu, přímo, v~popisku osy je označen \emph{excess risk}.
\par
...
Model se tedy nenaučil jen dobrý prediktor, ale skoro nejlepší možný.
...
Nebylo to tím, že by se model zasekl, ale tím, že dosedl těsně nad \emph{Bayes floor}, pod který se žádný prediktor nedostane.
\par

Za povšimnutí stojí, že uvnitř rozsahu je přebytečná chyba největší právě u~hladkých polí (Easy).
Z~grafu samotného vidíme jen tento výsledek, sloupec Easy je mezi režimy uvnitř rozsahu nejvyšší.
Vysvětlení nabízí spojení dvou dřívějších pozorování.
...
Hladší pole (OOD-long) zůstávají srovnatelná s~režimem Easy, kdežto u~členitějších polí (OOD-short) přebytečná chyba vyskočí na několikanásobek, tedy přesně tam, kde amortizace selhává.
\par

\begin{figure}[!htb]
    \centering
    \includegraphics[width=\linewidth]{figures/VariantC_pfn_seg/fig_C_06_bayes_floor.png}
    \caption{Vlevo chyba PFN a \emph{Bayes floor} (chyba samotného \emph{oraclu}) pro jednotlivé režimy, měřená binární křížovou entropií (BCE) na pixel. Vpravo jejich rozdíl $\mathrm{BCE}_{\mathrm{PFN}}-\mathrm{BCE}_{\mathrm{oracle}}$, v~grafu označený \emph{excess risk}, tedy přebytečná chyba PFN nad optimem. Uvnitř trénovacího rozsahu je malá, prudce roste jen u~členitějších polí mimo rozsah (OOD-short).}
...
Popišme nejprve, co obrázek~\ref{fig:seg-c-kolaps} zobrazuje a jak ho číst.
Zelená křivka je vzdálenost predikce od \emph{oraclu}, fialová vzdálenost od uniformní prior masky, obě jako funkce velikosti kontextu.
Kdyby kolaps nastával, viděli bychom při malém kontextu fialovou křivku klesat k~nule a zelenou stoupat, predikce by se totiž podobala prior masce, a ne pravdě.
Nic takového se neděje.
Zelená křivka je malá a plochá i při jediném \emph{support páru}, predikce je po celou dobu mnohem blíž pravdě než prior masce.
Kolaps zde tedy \textit{nenastává}.
Důvod je stejný jako v~části~\ref{subsec:seg-c-fidelita}, hustý vstupní obrázek prozradí úlohu i z~jednoho příkladu, takže model nemá proč se vracet k~průměru.
\par

Poznamenejme ještě, proč se fialová křivka drží u~hodnoty zhruba $0{,}5$ a s~kontextem se vůbec nehýbe.
Drtivá většina pixelů je jistý vnitřek nebo jisté pozadí a model tam predikuje hodnoty blízké nule nebo jedné.
Vzdálenost takové predikce od uniformní masky s~hodnotou $0{,}5$ je proto skoro přesně $0{,}5$ bez ohledu na kontext.
Nosným důkazem je tedy plochá a nízká zelená křivka, fialová jen dokládá, že se predikce uniformní masce nikdy nepřiblíží.
\par

Zdůrazněme, že tento negativní výsledek Naglerově teorii neodporuje.
Podmínka lokality (věta~\ref{theorem:lokalizace}) předpovídá tah k~prioru tam, kde je predikce daleko od dat.
Hustý vstupní obrázek ale tuto podmínku nikdy nesplní, model má pozorování po ruce v~každém pixelu a pravý posterior je proto daleko od prioru.
Kdyby PFN přesto kolaboval, nebylo by to potvrzení teorie, ale selhání aproximace posterioru.
Stopa tahu k~prioru nicméně zůstává i zde, je jí malé bias plató z~části~\ref{subsec:seg-c-bias-var}, které s~kontextem nemizí.
Tatáž síla, která u~řídkého kontextu způsobí kolaps, se u~hustého projeví jen touto drobnou systematickou odchylkou.
To zpřesňuje závěr ze sekce~\ref{sec:seg-universeg}, kolaps k~prioru není univerzální vlastnost amortizace, ale důsledek řídkého a málo informativního kontextu.
\par
...
Skutečné PFN natrénované na explicitním prioru věrně aproximuje pravý Bayesovský posterior, a to ve střední hodnotě i v~nejistotě.
Uvnitř trénovacího rozsahu je dokonce skoro Bayesovsky optimální, od nejlepšího možného prediktoru jej dělí jen malá přebytečná chyba.
Rozklad chyby přitom odpovídá Naglerově teorii, rozptyl s~kontextem mizí aspoň jako $n^{-1}$ a bias zůstává na malé, ale strukturální hodnotě, potvrzené shodou přes tři nezávislé tréninky.
\par
...
Hustý vstupní obrázek totiž úlohu prozradí i z~mála příkladů.
Oba experimenty této kapitoly tak vedou ke stejnému závěru.
Kolaps k~prioru u~předtrénovaného modelu ani selhání mimo trénovací rozsah nejsou vadou architektury, v~čistém režimu je totiž model prokazatelně skoro optimální.
