...
V~této kapitole dáme PFN těžší a praktičtější úlohu, a to segmentaci obrazu.
...
Tomuto rozhodování pro celý obrázek najednou se říká segmentace, tedy rozdělení obrazu na oblast zájmu a pozadí.
Oblasti zájmu, tedy pixelům hledaného útvaru, budeme říkat popředí (anglicky \emph{foreground}), všem ostatním pixelům pozadí (\emph{background}).
Výsledkem je tzv. maska, což je obrázek stejného rozměru jako vstup, ve kterém má pixel popředí hodnotu jedna a pixel pozadí hodnotu nula.
...
Model tedy nevrací rovnou tvrdou černobílou masku, ale pravděpodobnostní mapu, kde hodnota blízká jedné znamená „téměř jistě uvnitř“ a hodnota blízká nule „téměř jistě mimo oblast zájmu“.
...
Představme si nyní stejnou myšlenku, ale s~funkcí dvou proměnných, tedy s~náhodným polem přes celou plochu obrázku.
...
Ověříme, zda se u~něj objeví tzv. kolaps k~prioru.
...
Zde avšak proti ní můžeme přesně změřit, jak moc dobře ji PFN aproximuje a kde vzniká odchylka, tedy bias, rozptyl, kalibraci a chování mimo trénovací rozdělení.
...
Nahradíme ho tzv. operační prior maskou, tzn. průměrnou maskou přes všechny příklady dané domény.
...
\begin{figure}[H]
    ...
    \caption{Kolaps k~prioru. Zelená křivka je vzdálenost predikce od pravé masky $d_{\text{truth}}$, fialová vzdálenost od operační prior masky $d_{\text{prior}}$, obojí jako funkce velikosti \emph{support setu}. Při jednom příkladu je predikce blíž prioru než pravdě (kolaps), s~rostoucím kontextem se křivky protnou a predikce je blíže k~pravdě. Chybové úsečky jsou $\pm 1$ směrodatná chyba.}
    \label{fig:seg-a-kolaps}
\end{figure}

\textbf{Diskuze:}

Na obrázku~\ref{fig:seg-a-kolaps} kolapsem k~prioru rozumíme stav, kdy je predikce blíž prior masce než pravdě, tedy $d_{\text{prior}} < d_{\text{truth}}$.
Poznáme ho proto z~uspořádání obou křivek.
Zde tento stav nastává pouze při malém kontextu.
Při kontextu rovnajícím se jedné ukázce leží ve všech třech doménách fialová křivka pod zelenou.
S~rostoucím \emph{support setem} $d_{\text{truth}}$ prudce klesá, až se obě křivky protnou.
Křížení avšak neznačí kolaps samotný, ale jeho konec.
Od té chvíle je predikce blíž pravdě než prioru, vzdálenost k~pravdě přebije vzdálenost k~prioru.
Poloha křížení pak měří, kolik kontextu model k~tomuto úniku potřebuje.
Mírně se liší podle domény, nejdříve k~němu dochází u~snímků WBC-JTSC, nejpozději u~WBC-CV.
Kolaps tedy řídí velikost kontextu, nikoli konkrétní úloha.
Právě tato blízkost k~prioru při slabé podmiňující informaci Naglerova teorie předpovídá, a potvrzuje se tak i na modelu, který není úplně \emph{PFN-type}.
...
Obrázek~\ref{fig:seg-a-dice} ukazuje, že \emph{Dice} s~rostoucím kontextem roste a poté saturuje.
...
Poznamenejme, že tento závěr je v~souladu se vzorem, který jsme viděli i v~jedné dimenzi, model dobře pozná, kde je nejistý, ale absolutní míru nejistoty odhaduje hůře.
\par
...
\begin{figure}[p]
    ...
    \caption{Průměrná vzdálenost predikce PFN od \emph{oraclu} jako funkce velikosti kontextu pro pět režimů náročnosti. Uvnitř trénovacího rozsahu (Hard, Medium, Easy) je vzdálenost malá. U režimů Hard a Medium na velikosti kontextu téměř nezávisí, u režimu Easy s kontextem znatelně klesá. Mimo rozsah (OOD-short, OOD-long) je větší, členitější pole (OOD-short) se \emph{oraclu} nepřiblíží ani s~velkým kontextem.}
...
Důvod je také stejný.
Pole s krátkou korelační délkou obsahují jemnější útvary, než jaké model při tréninku viděl, a tento rozdíl nevyrovná ani hustý vstupní obrázek.
\par
...
Připomeňme, co je prior maskou.
Je to první predikce, kdy vstupní obrázek ještě nevidíme, ale známe pravidla této úlohy.
Pro každý pixel se tedy ptáme, s jakou pravděpodobností patří do popředí.
Tímto modelu říkáme, kolik zhruba oblasti popředí je na obrázku.
V naší úloze je odpověď pro všechny pixely stejná.
Skryté pole totiž vzniká v každém místě obrázku podle stejných pravidel (vzorkování z GP).
Útvary tvořené GP nemají žádný typický tvar, na rozdíl od např. krvinek.
Tedy jediná informace, kterou můžeme modelu sdělit, je celkový podíl popředí.
Zabírá-li popředí například třetinu obrázku, patří do něj každý pixel s pravděpodobností jedna třetina.
Prior maska je proto mapa se stejnou hodnotou ve všech pixelech, tedy jednobarevná plocha bez jakéhokoli tvaru.
...
Opět zkusme popsat, co tento na první pohled zvláštní graf zobrazuje na obrázku \ref{fig:seg-c-kolaps}.
Ve skutečnosti používáme zde stejný princip jako v \ref{fig:seg-a-kolaps}.
...
Podle popisu grafu \ref{fig:seg-c-kolaps} výše můžeme udělat následující závěr.
...
Většinu informace potřebné k~predikci tak model získá už ze samotného vstupního obrázku, z~jeho pixelů si totiž dokáže skryté pole zrekonstruovat, a tím zná i tvar hledané masky.
Ze \emph{support setu} se dozvídá jen to, co v~obrázku obsaženo není, tedy polohu prahu a sílu šumu (viz část~\ref{subsec:seg-c-fidelita}).
