...
Zelená křivka je vzdálenost predikce od \emph{oraclu}, fialová křivka je vzdálenost od uniformní prior masky (naš prior v tomto případě), obě jako funkce velikosti kontextu.
\par

\textbf{Diskuze:}
Podle popisu grafu výše můžeme udělat následující závěr.
Kdyby kolaps k prioru nastával i tady, viděli bychom fialovou křivku klesat k~nule při malém kontextu a zelenou naopak stoupat.
Predikce by se totiž podobala prior masce, a ne skutečnosti.
Avšak nic takového se tady neděje.
...
Predikce je tedy po celou dobu mnohem blíž pravdě než prior masce.
...
Podmínka lokality (věta~\ref{theorem:lokalizace}) předpovídá přiblížení k~prioru tam, kde je predikce daleko od dat.
Avšak hustý vstupní obrázek tuto podmínku nikdy nesplní, model má pozorování po ruce v~každém pixelu a pravý posterior je proto daleko od prioru.
Kdyby PFN přesto kolaboval, nebylo by to potvrzení teorie, ale selhání aproximace posterioru.
Stopa přiblížení k~prioru nicméně zůstává i zde.
Touto stopou je malé bias plató z~části~\ref{subsec:seg-c-bias-var}, které s~kontextem nemizí.
Stejná síla, která u~řídkého kontextu způsobí kolaps, se u~hustého projeví jen touto drobnou systematickou odchylkou.
To rozšiřuje závěr ze sekce~\ref{sec:seg-universeg}.
Kolaps k~prioru tedy není univerzální vlastnost amortizace, ale důsledek řídkého a málo informativního kontextu.
\par
...
Uvnitř trénovacího rozsahu je dokonce skoro Bayesovsky optimální.
Od nejlepšího možného prediktoru jej dělí jen malá chyba.
Rozklad chyby přitom odpovídá Naglerově teorii, rozptyl s~kontextem mizí aspoň jako $n^{-1}$ a bias zůstává na malé, ale strukturální hodnotě, a to bylo potvrzené shodou přes tři nezávislé tréninky.
\par
...
Hustý vstupní obrázek totiž úlohu prozradí i z malým počtem příkladů.
