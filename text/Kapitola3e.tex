...
\begin{table}[!htb]
    ...
    \hline
    \end{tabular}
    \caption{MSE vůči skutečným hodnotám jako funkce velikosti kontextu $n$ při misspecifikaci kernelu ($\ell_{\text{true}} = 0{,}3$, průměr přes $150$ instancí). PFN je pro $n \in \{10, 20, 40\}$ nejlepší ze všech misspecifikovaných metod, pro velká $n$ je s GP\_ML a GP\_Bayes prakticky shodné.}
...
\begin{table}[!htb]
    ...
    \hline
    \end{tabular}
    \caption{Kalibrace $\Delta$NLL vůči \emph{oracle} jako funkce velikosti kontextu $n$ ($\ell_{\text{true}} = 0{,}3$, průměr přes $150$ instancí, \emph{oracle} má z definice hodnotu $0$). PFN je nejblíže \emph{oracle} pro všechna $n \leq 40$, teprve od $n = 64$ jej mírně porazí GP\_ML.}
...
A to pro případy, které jsou hodně vzdálené od hranice trénovací množiny.
Avšak to opět může být způsobeno tím, že pro tato $\ell$ je tato úloha výrazně jednodušší.
