...
V praxi ale taková jistota neexistuje.
Skutečná data nepřicházejí se štítkem, ze kterého kernelu byla vygenerována.
Nyní proto pro PFN odstraníme i tuto poslední jistotu.
Data budeme generovat z tzv. Matérnova kernelu (definice viz dále), zatímco všechny porovnávané metody budou předpokládat RBF.
Skutečný generativní proces tak bude ležet mimo support prioru všech metod, včetně implicitního prioru PFN.
Budeme tedy zkoumat, co se stane, když po PFN chceme predikovat mimo support jeho prioru.
Bude nás zajímat o kolik se predikce zhorší a která metoda za těchto podmínek selže nejméně.
...
Data generujeme z GP s Matérnovým kernelem s korelační délkou $\ell_{\text{true}}$ a šumem $\sigma^2 = 0{,}1$.
...
Právě proto se Matérnův kernel považuje za realističtější model skutečných dat.
Nekonečná hladkost RBF je totiž pro reálné procesy příliš silný předpoklad~\cite{rasmussen2006}.
...
Pro $\nu \to \infty$ přechází v RBF, oba kernely jsou si tedy velmi podobné.
Situace nebo omyl, kdy data pocházející z Matérnova kernelu modelujeme pomocí RBF, se proto může snadno přihodit i v praxi.
Současně jsou oba kernely dost odlišné na to, aby tento omyl žádná volba hyperparametrů RBF nedokázala úplně napravit.
...
Adaptivní metody GP\_ML a GP\_Bayes si korelační délku mohou z dat znovu odhadnout, jejich handicap tedy s daty mizí.
...
Z tabulek~\ref{tab:exp7_mse_n} a~\ref{tab:exp7_dnll_n} plynou dvě pozorování.
...
Za druhé, v kalibraci je obraz ještě víc jednoznačněji.
PFN má nejmenší $\Delta$NLL ze všech misspecifikovaných metod pro všechna $n \leq 40$ i pro $n = 100$, jak ukazuje tabulka~\ref{tab:exp7_dnll_n}.
U velkých kontextů už mezi PFN a GP\_ML není jednoznačné pořadí, GP\_ML vede při $n = 64$ a $n = 128$.
...
Za povšimnutí stojí i chování obou krajních metod.
GP\_fixed sice s rostoucím kontextem svou bodovou chybu snižuje, jeho $\Delta$NLL ale podle tabulky~\ref{tab:exp7_dnll_n} s daty neklesá, šířku prediktivních intervalů totiž určuje pevně zvolený kernel, na který žádné pozorování nesáhne.
Naopak GP\_Bayes je při malých kontextech kalibrovaný znatelně lépe než GP\_ML, přestože jeho prior pravé hodnotě korelační délky téměř nevěří.
Jediný rozdíl mezi nimi je přitom marginalizace, GP\_ML sází vše na jediný bodový odhad korelační délky, GP\_Bayes přes její nejistotu průměruje.
Vidíme tedy stejný mechanismus, který pomáhá PFN, jen v menším měřítku.
\par
...
\begin{table}[!htb]
    ...
    \hline
    \end{tabular}
    \caption{MSE vůči skutečným hodnotám jako funkce velikosti kontextu $n$ při misspecifikaci kernelu ($\ell_{\text{true}} = 0{,}3$, průměr přes $150$ instancí). PFN je pro $n \in \{5, 20, 40, 64\}$ nejlepší ze všech misspecifikovaných metod, pro velká $n$ je s GP\_ML a GP\_Bayes prakticky shodné.}
...
\begin{table}[!htb]
    ...
    \hline
    \end{tabular}
    \caption{Kalibrace $\Delta$NLL vůči \emph{oracle} jako funkce velikosti kontextu $n$ ($\ell_{\text{true}} = 0{,}3$, průměr přes $150$ instancí, \emph{oracle} má z definice hodnotu $0$). PFN je nejblíže \emph{oracle} pro všechna $n \leq 40$ i pro $n = 100$, GP\_ML jej mírně předstihne jen při $n = 64$ a $n = 128$.}
    \label{tab:exp7_dnll_n}
\end{table}

\begin{figure}[!htb]
    \centering
    \includegraphics[width=0.70\linewidth]{figures/GP2_exp7_misspecification/fig_exp7_q1_mse_vs_n_wide.png}
    \caption{Vliv velikosti kontextu při misspecifikaci kernelu (data z Matérn $2{,}5$, $\ell_{\text{true}} = 0{,}3$). Vlevo: MSE jako funkce $n$, černá tečkovaná čára je \emph{GP\_oracle}. Vpravo: $\Delta$NLL vůči \emph{oracle}, tečkovaná čára značí nulu (\emph{oracle}). Chybové úsečky $\pm 1$ směrodatná chyba. PFN (modrá) se v MSE drží těsně u \emph{oracle} a v $\Delta$NLL je pro malé a střední kontexty ze všech metod nejníže.}
    \label{fig:exp7_q1}
\end{figure}
...
\begin{table}[H]
    ...
    \hline
    \end{tabular}
    \caption{Kalibrace $\Delta$NLL vůči \emph{oracle} jako funkce $\ell_{\text{true}}$ ($n = 40$, průměr přes $150$ instancí). Pro střední korelační délky kolem pravé hodnoty $0{,}2$ až $0{,}3$ je nejblíže \emph{oracle} PFN. Pro velmi krátké i dlouhé délky vedou adaptivní metody, $\Delta$NLL PFN však zůstává malé v celém rozsahu, a to i mimo jeho trénovací rozsah $\ell > 1{,}0$.}
...
Pro extrémně krátké délky ho přebijí ostatní adaptivní metody, kterým hrubá data poskytují silný signál pro odhad $\ell$.
...
\begin{figure}[!htb]
    \centering
    \includegraphics[width=0.92\linewidth]{figures/GP2_exp7_misspecification/fig_exp7_q2_mse_vs_ls_wide.png}
    \caption{Vliv síly misspecifikace ($n = 40$). Vlevo: MSE jako funkce $\ell_{\text{true}}$. Vpravo: $\Delta$NLL vůči \emph{oracle}. Modrý pás značí trénovací rozsah PFN $\ell \in [0{,}05,\, 1{,}0]$. GP\_fixed (oranžová) pro krátké korelační délky selhává v obou metrikách, ostatní metody se s rostoucím $\ell_{\text{true}}$ sbíhají k \emph{oracle}. PFN zůstává blízko \emph{oracle} v celém rozsahu včetně hodnot mimo svůj trénovací rozsah.}
    \label{fig:exp7_q2}
\end{figure}
...
Dále bychom chtěli, aby korelační délka opustila trénovací rozsah na obou stranách.
...
\hline
\end{tabular}
\caption{Chování PFN mimo trénovací rozsah korelační délky (RBF data, $n = 40$, $\sigma^2 = 0{,}01$, průměr přes $200$ instancí). Relativní MSE je odchylka PFN od \emph{oracle} dělená chybou, kterou má sám \emph{oracle} kvůli šumu v datech. Minimum leží uprostřed trénovacího rozsahu a ani mimo rozsah na žádné straně relativní MSE nepřekročí referenční hodnotu $1$.}
\label{tab:exp7_ood}
\end{table}
...
Přesnou příčinu polohy minima neurčujeme, prior nad $\ell$ je rovnoměrný, takže minimum nelze vysvětlit tím, že by model střední délky viděl při tréninku častěji.
Zde jde spíše o~souhru obtížnosti úlohy a normalizace \emph{oraclem}.
\par

Za krátkým okrajem rozsahu chyba prudce vzroste.
Krátké realizace se rychle mění, kontext $40$ bodů je nedokáže zachytit a navíc jde o~délky, které model při tréninku neviděl.
Zajímavé je, že pro úplně nejkratší délky už relativní chyba dále neroste, dokonce mírně klesá, jak je vidět v~tabulce~\ref{tab:exp7_ood}.
Nejde o~to, že by se PFN zlepšovalo.
V~tomto režimu je úloha prakticky nepredikovatelná a velkou chybu dělá i samotný \emph{oracle}, jmenovatel relativní MSE proto rychle roste.
Obě metody se zároveň uchylují ke stejné opatrné predikci blízké apriorní střední hodnotě, tedy nule, a jejich vzájemná vzdálenost se přestává zvětšovat.
Relativní chyba se tak ustálí hluboko pod referenční hodnotou, ne proto, že by PFN úlohu zvládlo, ale proto, že ji v~tomto režimu nezvládá nikdo.
\par

Za dlouhým okrajem chyba naopak opět klesá.
Pro velmi dlouhá $\ell$ jsou realizace hladké a úloha je snadná i pro PFN, které takové délky při tréninku nevidělo.
Referenční hodnotu chyba nepřekročí na žádné straně, a to ani pro případy hodně vzdálené od hranice trénovací množiny.
Odchylka PFN od \emph{oracle} tedy všude zůstává menší než chyba způsobená samotným šumem v datech.
...
Pro hladké funkce za horní hranicí rozsahu to dává dobrý smysl.
Model se během tréninku naučil reprezentovat hladkost a delší korelační délka pro něj není kvalitativně nová situace.
Tím je zpětně podložený i závěr předchozí části.
Plynulá degradace mimo trénovací rozsah není artefakt misspecifikovaného kernelu, platí to i v čistém testu.
\par

\begin{figure}[!htb]
    \centering
    \includegraphics[width=0.92\linewidth]{figures/GP2_exp6_hp_identification/fig_exp6_q4_ood.png}
    \caption{Relativní MSE jako funkce pravé korelační délky $\ell$ (logaritmická osa). Zelený pás značí trénovací rozsah $\ell \in [0{,}05,\, 1{,}0]$, zelená tečkovaná čára referenční hodnotu $\text{rel\_MSE} = 1$. Křivka má minimum uprostřed trénovacího rozsahu a k jeho okrajům roste. Za krátkým okrajem chyba prudce vzroste a ustálí se, za dlouhým okrajem opět mírně klesá. Referenční hodnotu nikde nepřekročí.}
    \label{fig:exp7_ood}
\end{figure}
...
GP\_fixed, který nemá žádný mechanismus adaptace, selhává nejvíce pro krátké a střední korelační délky a v MSE pro všechna $n$.
Pro dlouhé korelační délky, kde se jeho pevné $\ell = 0{,}7$ blíží pravé hodnotě, přestává být nejhorší.
