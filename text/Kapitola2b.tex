...
V této sekci uvedeme dosud známé statistické vlastnosti PFN, které publikoval Nagler v článku~\cite{nagler2023}.
...
Müller ve své práci ukázal empirická pozorování, která ale nepodpořil teoretickým zdůvodněním.
To se avšak podařilo Naglerovi, který dokázal vybudovat teoretický základ pro PFN a řádně odůvodnit, proč a jak architektura PFN dává dobré výsledky při Bayesovské inferenci.
...
Z hlediska Bayesovského neparametrického odhadování můžeme na $p_0$ nahlížet jako na realizaci náhodného parametru $p \in \mathcal{P}$, kde $\mathcal{P}$ je prostor podmíněných pravděpodobnostních rozdělení nad $\mathcal{Y} \times \mathcal{X}$.
Tento parametr má rozdělení $\Pi$, kterému budeme říkat prior.
...
Tento mechanismus definuje sdružené rozdělení nad $(D_n \cup (Y, X), p)$ pro každé $n$. 
...
kde $\Pi(\cdot \mid D_n)$ je tzv. \textit{posterior}, což je podmíněné rozdělení $p$ dané pozorovanými daty $D_n$, získané Bayesovým pravidlem. 
...
Platí–li podmínka, pak když jsme viděli data $D_n$, říkají nám něco o $p(y \mid x)$, ale $x$–ová souřadnice testovacího bodu sama o sobě nic neříká o $p(y \mid x)$.
...
PPD $\pi(y \mid x, D_n)$ je tedy posteriorní střední hodnota podmíněných rozdělení $p(y \mid x)$, vážená tím, jak jsou jednotlivé modely $p$ konzistentní s $D_n$.
\par   
...
\begin{theorem}[Nagler~\cite{nagler2023}, Thm 2.2]
    $\pi$ z rovnice \ref{def:PPD} splňuje:
    \begin{equation}
            \pi = \arg\max_{q \in \mathcal{Q}} \, \mathbb{E}_\Pi[\log q(Y \mid X, D_n)],
    \end{equation}
    kde $\mathbb{E}_\Pi$ je střední hodnota přes $(Y, X) \cup D_n$ generované dle mechanismu popsaného v~sekci~\ref{subsec:statisticky-model}.
...
V tom případě rovnice \ref{eq:KL_optimalni_parametry} definuje KL-optimální aproximaci $\pi$ ve třídě $\{q_\theta : \theta \in \Theta\}$, tj. nejlepší aproximaci, které je daná architektura schopna.

Dále lze ukázat následující teoretickou vlastnost.
Střední hodnota v~rovnici~\ref{eq:KL_optimalni_parametry} se v praxi aproximuje průměrováním přes $m$ iid datasetů, např. pomocí metody Monte Carlo.
...
Nagler~\cite{nagler2023} ukazuje, že pokud je trénovací loss NLL (negative log-likelihood, viz algoritmus~\ref{alg:nll_loss}) a model má dostatečnou kapacitu, optimální řešení konverguje k Bayesovskému posteriornímu prediktivnímu rozdělení. To znamená, že správně natrénovaný PFN je teoreticky ekvivalentní analytickému GP posterioru.


\subsection{Od PPD k PFN}
\label{subsec:ppd-k-pfn}
V této sekci si ukážeme podrobněji jak spolu souvisí pojmy PPD a PFN, řekneme si zajímavé vlastnosti ohledně učení PPD, které spojují tyto dva nástroje, a jako důsledek popíšeme i vlastnosti učení samotného PFN.
...
Avšak v tomto okamžiku čelíme velmi netriviálnímu problému, a to jak zvolit správný prior, aby to konvergovalo.
...
Např. pro případ GP s RBF kernelem bychom chtěli pokrýt celý prostor všemožných tvarů té funkce, tj. aby model uměl najít správné hyperparametry v RBF kernelu.
Tím pádem, když PPD dostane sadu bodů, dokáže nám říct, ze které konkrétní konfigurace GP s RBF kernelem pochází.
Ale ani velký support nemusí stačit.
...
\begin{theorem}[Nagler~\cite{nagler2023}, Thm 3.1]
...
Při tréninku (viz algoritmus~\ref{alg:pfn_training}) se vzorkují datasety $D^{(j)}$ s náhodnou velikostí $N_j \sim \Pi_N$. 
...
Rozdělení $\Pi_N$ tak určuje, které velikosti datasetů jsou více signifikantní. 
...
V předchozí sekci jsme ukázali podstatnou vlastnost PPD, a to, že se se zvětšujícím se datasetem PPD přibližuje ke skutečnému rozdělení.
...
Pro libovolnou velikost datasetu $n$ při inferenci nahlížíme na předtrénovaný model $q_{\hat\theta}(y \mid x, \cdot)$ jako na nenatrénovaný frekventistický prediktor pro $p_0(y \mid x)$, což je funkce $(\mathcal{Y} \times \mathcal{X})^n \to \mathcal{P}_{\mathcal{Y}|\mathcal{X}}$, která mapuje dataset $D_n$ na podmíněné rozdělení.
Parametry $\theta$ jsou pak jen hyperparametry tohoto prediktoru, vyladěné před tréninkem. 
Prior $\Pi$ a $\Pi_N$ jsou jednoduše rozdělení nad úlohami, na kterých chceme, aby prediktor fungoval dobře.
...
Takže zkusme najít které strukturální aspekty PFN by to mohly vysvětlit.
\par 
...
\begin{theorem}[Nagler~\cite{nagler2023}, Lemma 5.1]
...
Formálně předpokládáme, že existují $\alpha > 0$ a $L < \infty$ takové, že pro dostatečně velká $n$ a s. j. datasety $D_n$, $D'_n$ lišící se v jediném vzorku:
\begin{equation}
...
\begin{theorem}[Nagler~\cite{nagler2023}, Thm 5.2]
    ...
    \begin{equation}
        q_\theta(y \mid x, D_n) - \mathbb{E}[q_\theta(y \mid x, D_n)] \xrightarrow{n \to \infty} 0 \quad \text{s. j.}
    \end{equation}
\end{theorem}
Jinými slovy rozptyl postupně mizí s počtem dat $n$.
Tak dostáváme, že hlavním pramenem chyby při inferenci je bias, kterého se už jen tak nezbavíme.
\par   

Ze vztahu \ref{Bias_Variance} je vidět, že bias je určen chováním posloupností $\mathbb{E}[q_\theta(y \mid x, D_n)]$.
Je logické, že bychom mohli předpokládat (nebo očekávat), že platí i tento vztah: 
...
Ale co můžeme říct s jistotou, tak že chování biasu hodně závisí na oné architektuře, jak ukazuje Nagler ve svém článku~\cite{nagler2023}, může růst, klesat, anebo být celou dobu konstantní.
Co je avšak v našich silách, je vyslovit nutnou podmínku pro mizení biasu. 
...
\begin{theorem}[Nagler~\cite{nagler2023}, Thm 5.4]
    Nechť $\mathcal{P}$ je třída rozdělení taková, že pro každé $p \in \mathcal{P}$:
...
\begin{equation}
    \left| q_\theta(y \mid x, D_n) - q_\theta(y \mid x, \tilde{D}_n) \right| \xrightarrow{n \to \infty} 0, \quad \text{s. j.}
\end{equation}
...
To znamená, že množina $\tilde{D}_n$ je poskládaná tak, že místo vzdálených od $x$ bodů obsahuje náhodný šum, ale podstatné je, že nakonec ten šum (odlehlé body) vůbec nemá vliv na výslednou predikci.
...
Přepis labelů vzdálených vzorků proto vždy predikci ovlivní, a lokalita ve smyslu věty \ref{theorem:lokalizace} není splněna.
...
Jelikož tato práce je věnovaná pohledu na PFN skrz architekturu transformer~\cite{vaswani2017}, dává smysl podívat se na statistické vlastnosti právě této architektury.
...
Nejprve \emph{attention} část:
\begin{gather*}
a_j^{(h)} = \mathrm{SoftMax}\left(v^\top W_q^{(h)}V_1, \dots, v^\top W_q^{(h)}V_n\right)_j, \\[1ex]
u' = \sum_{h=1}^H \sum_{j=1}^n a_j^{(h)} W_v^{(h)} V_j.
\end{gather*}
Na ni navazuje dopředná síť s normalizacemi a výstupní vrstvou:
\begin{gather*}
u = \mathrm{LayerNorm}(v + u'; \gamma), \\[1ex]
z' = W_{r,2}\mathrm{ReLU}(W_{r,1}u; \gamma), \\[1ex]
z = \mathrm{LayerNorm}(u + z'; \gamma), \\[1ex]
...
První dvě rovnice definují \emph{attention mechanismus} s $H$ hlavami~\cite{vaswani2017}.
Ze součtu $a^{(h)}_1 + \cdots + a^{(h)}_n = 1$ plyne, že \emph{attention váhy} tvoří pravděpodobnostní rozdělení přes trénovací vzorky.
...
\begin{theorem}[Nagler~\cite{nagler2023}, Thm 6.2]
...
\begin{theorem}[Nagler~\cite{nagler2023}, Thm 6.3]
...
Navíc v definici $\hat{u'}$ se objevuje tzv. \emph{exponentially tilted function} (česky exponenciálně vychýlená funkce).
Jde o standardní transformaci z teorie pravděpodobnosti.
Původní rozdělení $p_0(s)$ se přenásobí exponenciálním faktorem $\exp(v^\top W^{(h)}_q s)$ a znormuje, čímž se jeho pravděpodobnostní hmota přesune k hodnotám $s$ s velkým skalárním součinem $v^\top W^{(h)}_q s$.
V tomto kontextu se objevuje proto, že přesně takto se chovají \emph{attention váhy} v limitě nekonečně mnoha vzorků, je tedy asymptotickou variantou \emph{attention mechanismu}.
...
Jinými slovy ten složitý vzoreček výše ukazuje teoretickou situaci, co by se stalo, kdyby měl transformer k dispozici nekonečně mnoho vzorků. 
Už nerozděluje váhy mezi jednotlivými datovými body, ale vezme celé spojité rozdělení dat a „vychýlí“ je celé směrem k té informaci, na kterou zrovna potřebuje upřít pozornost.
Více formálně to znamená, že každá \emph{attention hlava} $h$ tak hodnotí určitý aspekt neznámého rozdělení $p_0$ (charakterizovaný maticí $W^{(h)}_q$). 
...
Zvyšuje váhu vzorků podobných $v$, ale ne ve smyslu věty \ref{theorem:lokalizace}.
Vzorky vzdálené od $x$ stále nenulově přispívají, protože SoftMax nikdy nevytváří přesně nulové váhy. 
Přepsání labelů vzdálených vzorků proto vždy predikci změní, a tak lokalita podle \ref{theorem:lokalizace} není splněna.
...
Relevance jednotlivých aspektů závisí na pravém rozdělení $p_0$.
...
Lokalizace tedy zlepšuje bias za cenu mírného zvýšení rozptylu.
