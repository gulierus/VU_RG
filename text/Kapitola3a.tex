...
Nagler se ve své práci snaží matematicky popsat principy PFN.
Uvažuje však model s jednou vrstvou a to, co PFN transformer ve výsledku dělá a jak fungují jeho vnitřní mechanismy, popisuje jen částečně.
Proto se pokusíme tyto otevřené otázky prozkoumat, když ne matematickým aparátem, tak alespoň empirickými pozorováními.
Všechny experimenty provedeme jen pro architekturu typu transformer.
Zajímat nás bude jak vnitřní struktura PFN, tak i některé jeho vlastnosti jako nástroje pro Bayesovskou inferenci.
Rovněž prozkoumáme mechanismy, kterými PFN vyhodnocuje vstupní data, a ukážeme, čím se tyto mechanismy liší od regrese pomocí Gaussovských procesů.
...
Zaměříme se zde na to, co přesně dělá se vstupními daty.
Podíváme se, zda se snaží napodobit stejné mechanismy jako GP, anebo zda dělá něco jiného.
Experimenty jsme prováděli na dvou typech modelů.
Oba sdílejí stejnou architekturu transformeru: $6$ vrstev, $8$ \emph{attention hlav}, dimenze embeddingu $512$, dimenze skrytých vrstev dopředné sítě $1024$ a výstupní \texttt{FullSupportBarDistribution} s $1000$ biny.
...
Liší se však tím, z jakého prioru nad hyperparametry trénovací data pocházela.
S tím souvisí i délka tréninku, pro nefixní hyperparametry totiž model potřebuje více času na učení.
...
Model byl trénován na GP, jehož hyperparametry byly při každé dávce vzorkovány nezávisle z předem daného rozdělení: $\ell \sim \mathcal{U}(0{,}05,\, 1{,}0)$, $\text{outputscale} \sim \text{LogNormal}(0,\, 0{,}5)$, $\sigma^2 \sim 10^{\mathcal{U}(-3,\,-1)}$.
Tento model se tedy musí naučit generalizovat přes celou rodinu GP funkcí s různými korelačními délkami a amplitudami.
Jde tedy o plnohodnotný \emph{meta-learning} nad rozdělením hyperparametrů.
Právě proto vyžaduje podstatně delší trénink.
Model viděl nespočet různých tvarů funkcí a musel se naučit rozpoznávat hyperparametry přímo z kontextových dat.
...
Začneme ovšem případem fixních hyperparametrů.
Pro model to je značné zjednodušení, přesně ví, co se má naučit.
Jak se ukáže dále, naučí se to velmi efektivně.
...
Začneme tím, že si vykreslíme jednotlivé vrstvy.
Podíváme se tak dovnitř transformeru na to, jak jeho vrstvami prochází tok dat.
\emph{Attention matice} je přirozené okno do toho, jak transformer interně organizuje informaci.
Připomeňme, že v naší konfiguraci má model $6$ vrstev a v každé vrstvě $8$ \emph{attention hlav}, přičemž každá hlava může sledovat jiný aspekt vstupních dat.
...
Každá vrstva přitom neobsahuje jen jednu takovou matici, ale $8$, tj. jednu pro každou \emph{attention hlavu}, protože každá hlava má vlastní naučené váhy.
Abychom nemuseli sledovat všech $8$ matic zvlášť (pro $6$ vrstev bychom obdrželi $48$ matic), sečteme je po prvcích a vydělíme osmi, čímž z nich uděláme jedinou průměrnou matici pro každou vrstvu.
...
Z pohledu GP inference bychom očekávali, že právě tento kvadrant bude dominantní.
Predikce v novém bodě $x^*$ totiž závisí na pozorovaných párech $(X, y)$, nikoli naopak.
\par
...
Tuto vrstvu můžeme interpretovat jako explorativní fázi (neboli poznávací), tzn. model sbírá globální informaci o rozložení všech bodů v sekvenci a podrobněji jejich vzájemné vazby nezkoumá.

\textbf{Vrstvy 1–4:}

\emph{Attention} postupně řídne, váhy se soustřeďují na méně pozic a zbytek klesá k nule.
Lze zde pozorovat tvorbu určitých struktur ve vzorech.
Kvadrant \textit{Test$\to$Train} začíná nabývat na intenzitě, zatímco \textit{Train$\to$Test} slábne.
...
Podstatné je, že tuto asymetrii model neměl explicitně zakódovanou, a tudíž vyplynula přirozeně z optimalizace na syntetických GP datech.
Kdybychom totiž model trénovali na datech bez této struktury, \emph{attention mechanismus} by zůstal symetrický.
Samotná existence asymetrie v poslední vrstvě je tak prvním experimentálním dokladem, že PFN provádí Bayesovskou inferenci, nikoli pouhý \emph{pattern matching}, neboli učení příkladů nazpaměť.
\par
...
Za prvé, softmax nutí každý řádek matice, aby se sečetl na jedničku.
Každý \textit{query} bod tedy musí své \emph{attention} někam rozdělit, i když pro něj žádný klíč není skutečně relevantní.
Typicky se to stává v~raných vrstvách, než se vybudují informativní reprezentace, nebo u~bodů daleko od dat.
Přebytek \emph{attention} se pak přerozdělí na několik stále stejných pozic, které vytvoří svislé pruhy.
...
Dá se tedy říct, že existence pruhů je dalším nepřímým dokladem, že PFN \emph{attention} není prostý \emph{kernel smoothing}.
...
Klíčovou roli hraje inverze $[\mathbf{K} + \sigma^2 \mathbf{I}]^{-1}$, která dekoreluje blízké trénovací body.
Body, které si jsou podobné, si vzájemně „konkurují“ a jejich kumulativní vliv je potlačen.

\textbf{Nadaraya-Watsonův estimátor:}

Nejjednodušší alternativou, jak odhadnout GP, je Nadaraya-Watsonův (NW) estimátor~\cite{bishop2006}, tj. normalizovaný vážený průměr trénovacích hodnot:
...
Tento model je příliš velkým zjednodušením, pokud bychom ho chtěli použít pro odhad GP.
NW totiž implicitně předpokládá, že trénovací body jsou vzájemně nekorelované.
Blízké body proto dostávají neúměrně velkou kumulativní váhu a predikce jsou pak příliš hladké.
Současně je ale velmi přirozenou alternativou, jak GP odhadovat.
Právě proto dále budeme zkoumat, zda se transformer nenaučil jen tuto jednoduchou cestu.
...
Klíčová otázka, kterou budeme v následujících experimentech testovat, pak zní: odpovídají \emph{attention váhy} PFN normalizovaným RBF vahám, nebo se model naučil odlišnou strategii?
...
V této sekci se zaměříme na to, zda se PFN transformer snaží napodobit chování RBF kernelu, a proto GP tak dobře aproximuje, anebo zda dělá něco jiného.
...
Za prvé, korelace \emph{attention vah} s RBF kernelem je nízká ($0{,}374 \pm 0{,}270$) a má vysoký rozptyl.
Pouze pro některé realizace je ta korelace větší než $0{,}5$, a to stále nepovažujeme za dostatečné, abychom mohli říct, že kopírujeme chování RBF.
Tímto usuzujeme, že model neprovádí prostý RBF \emph{kernel smoothing}.
...
Obě tyto metody predikují zcela odlišné hodnoty.
Dále je vidět, že je PFN $1\,256\times$ přesnější než NW (poměr $\textrm{MSE(NW,\,GP)} / \textrm{MSE(PFN,\,GP)} = 0{,}2099 / 0{,}000167$).
\par
...
Nás by ale mohlo zajímat, proč je \emph{attention} ostřejší než RBF, tj. proč \emph{attention váhy} nejsou rozloženy tak hladce a plynule jako RBF váhy na obrázku~\ref{fig:attn_vs_rbf}.
...
A pro ten může být výhodnější jiná strategie, kde se bod dívá na sousedy s jinou silou.
Dokonce se ukazuje, že se může dívat na vzdálenější body víc než na své blízké sousedy.
Na konci pak FFN snáze provede korekci odpovídající efektu $\mathbf{K}^{-1}$.
\par

\textbf{Metodologická poznámka k této analýze:}

Nízká korelace \emph{attention vah} s RBF kernelem je platný a hodnotný závěr o chování jedné komponenty sítě.
Existují však důvody, proč z ní nelze přímo usuzovat na celkové chování modelu.

Za prvé, \emph{attention} není výstupní mechanismus, po něm následuje FFN, reziduální spojení a v případě vícevrstvého modelu dalších pět bloků.
I kdyby \emph{attention váhy} v poslední vrstvě přesně odpovídaly normalizovaným RBF vahám, výstup PFN by se od NW stále lišil vlivem korekcí ve FFN.
...
Oba modely (PFN i \emph{GP oracle}) budou predikovat skutečné šumové hodnoty GP realizace, kterou neviděly.
...
Z těchto $100$ bodů náhodně vybereme $n_{\text{ctx}}$ jako trénovací kontext, tj. dvojice $(x, y)$, které oběma metodám ukážeme jako pozorovaná data.
Zbývajících $100 - n_{\text{ctx}}$ bodů necháme skryté a slouží jako testovací cíl, jejich $x$ předložíme modelu k~predikci, ale jejich pravé $y$-hodnoty mu neprozradíme.
...
Slouží tedy jako horní hranice toho, čeho lze na daných datech dosáhnout, se kterou PFN porovnáváme.
...
Za druhé, rozdíl se zvětšuje u středních hodnot $n_{\text{ctx}}$.
Při $n = 5$ je převaha \emph{GP oracle} malá ($0{,}034$ vs $0{,}041$), při $n = 15$ již GP dosahuje $0{,}003$ oproti $0{,}006$ u PFN.
Příčinou je, že s více kontextovými body může \emph{GP oracle} využít přesnou inverzi kernelové matice $\mathbf{K}^{-1}$, zatímco PFN tuto operaci pouze aproximuje.
Při malém kontextu je $\mathbf{K}^{-1}$ málo informativní a obě metody jsou si blízko.

Za třetí, od $n = 20$ se obě metody přibližují, jejich MSE ($0{,}0021$ vs $0{,}0017$) jsou prakticky rovnocenné.
PFN tedy s dostatečným kontextem konverguje k chování \emph{GP oracle}, aniž by explicitně invertovalo kernelovou matici.
Toto je jedním z klíčových empirických potvrzení toho, že PFN provádí plnou GP inferenci namísto pouhého \emph{kernel smoothingu}.

\subsection{Konvergence střední hodnoty a kalibrace rozptylu}
\label{subsec:konvergence-variance}

V tomto experimentu ověřujeme, zda PFN správně zachytí nejen střední hodnotu predikce, ale také tvar profilu nejistoty (tzv. \emph{uncertainty}), tedy kde si je model jistější a kde méně.
...
Výsledek experimentu jsme opět získali průměrováním přes $5$ realizací $\times$ $5$ \emph{seedů} $= 25$ instancí celkem.
...
Avšak poznamenejme, že pro realizace s vysokou amplitudou ($|m_{\text{train}}| > 1{,}5$) dochází k neúplné konvergenci, GP se v takových případech chová stejně.
...
Podle tabulky \ref{tab:exp1} korelace $r = 0{,}98$ potvrzuje, že PFN správně identifikuje tvar profilu \emph{uncertainty}, tedy kde je nejistota větší a kde menší.
