...
Za prvé, softmax nutí každý řádek matice sečíst na jedničku – každý \textit{query} bod tedy \emph{musí} své \emph{attention} někam rozdělit, i když pro něj žádný klíč není opravdu relevantní (typicky v~raných vrstvách, než se vybudují informativní reprezentace, nebo u~bodů daleko od dat).
...
\begin{figure}[htbp]
    \centering
    \subcaptionbox{MSE PFN (modrá) a \emph{GP oracle} (zelená přerušovaná) jako funkce $n_{\text{ctx}}$. Chybové úsečky $= \pm 1$ \emph{std} přes $20$ realizací. \emph{GP oracle} mírně vede od $n = 10$, protože zná správné $\ell$.\label{fig:exp4_mse}}[0.82\linewidth]{%
...
Příčinou je, že s více kontextovými body disponuje \emph{GP oracle} přesnou inversí kernelové matice $\mathbf{K}^{-1}$, zatímco PFN tuto operaci pouze aproximuje.
