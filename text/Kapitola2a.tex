...
\begin{itemize}
    ...
    \item \lstinline|attention_between_features = False| – slouží pro zpracování tabulkových dat; pokud je zapnutý, jedná se o architekturu \textbf{TabPFN}, kterou v této práci neuvažujeme (viz Hollmann et al., 2022).
\end{itemize}
...
Parametr \texttt{features\_per\_group} určuje, kolik sloupců se seskupí do jednoho tokenu.
...
Výsledkem je jeden token pro jednu konkrétní pozici, tj. tento token kombinuje informaci o poloze $x$ a hodnotě $y$ v jednom vektoru.
...
Jakmile embeddingy vstupují do transformer vrstev je zvykem jim říkat token. 
...
Jsme si již řekli, že PFN je encoder–only architektura, tj. decoder blok, který je součástí obecné architektury transformer, v něm chybí.
