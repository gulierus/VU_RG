...
- **Dvě ortogonální osy degradace** (drženy odděleně, A.4):
  1. `n_supp ∈ {1, 2, 4, 8, 16, 32, 64}`;
  2. OOD-ness domény: **WBC-JTSC** (kotva/ID) → **WBC-CV** (mírné OOD, jiná akvizice) →
     **OASIS** (silné OOD, jiná modalita i anatomie).
...
(context size → error saturace) a plató popsané v UniverSeg. OOD-ness sráží plató (JTSC 0,97 >
CV 0,92); OASIS má vyšší Dice při `n=1` díky velké, konzistentně umístěné struktuře.
...
- `fig_A_02_dice.png` — Dice vs `n_supp` (saturace + OOD-ness).
