...
| **Random-HP model** | `pfn_rand_hps_latest_epoch_500.pth` | 500 | $\ell \sim U(0.05, 1.0)$, $\sigma^2 \sim 10^{U(-5,-2)}$ | 6 vrstev, 14.14M param |
...
- **Marginalizace:** průměr GP predikcí přes 30 hodnot $\ell \in [0.05, 1.0]$, váhy rovnoměrné
...
> Funkce `run_exp1(rand_model, HPS_BIG, seed)` — stejný protokol jako výše, 5 seedů × 5 realizací = **25 hodnot**.  
...
![Vzorky z GP prioru — 100-epoch model](figures/fig_big_01_prior_samples.png)
*Obrázek 1: Vzorky z GP prioru ($\ell=0.3$, $n=20$ trénovacích bodů). PFN je trénován na těchto realizacích.*

![PFN vs GP uncertainty — 100-epoch model](figures/fig_big_02_uncertainty.png)
...
![Vzorky z GP prioru — random-HP model](figures/fig_rand_01_prior_samples.png)
*Obrázek 3: Vzorky z GP prioru pro random-HP model ($\ell=0.3$, různé realizace).*

![PFN vs GP uncertainty — random-HP model](figures/fig_rand_02_uncertainty.png)
...
> **Zdroj dat:** Výstupy notebooků `experiments/Experiments_from_GP1_big_model.ipynb` (cell 7) a `Experiments_from_GP1_random_model.ipynb` (cell 6), extrahované do `background/figures/` skriptem `background/extract_figures.py`.  
...
![Attention matice — všech 6 vrstev, 100-epoch model](figures/fig_big_03_attention_all_layers.png)
...
![Attention matice — všech 6 vrstev, random-HP model](figures/fig_rand_03_attention_all_layers.png)
...
![Attention detail + RBF + entropie — 100-epoch model](figures/fig_big_04_attention_detail.png)
...
![PFN vs NW vs GP predikce — 100-epoch model](figures/fig_big_05_nw_comparison.png)
...
![Attention detail — random-HP model](figures/fig_rand_04_attention_detail.png)
...
![MSE vs n_context — 100-epoch model](figures/fig_big_06_context_size_mse.png)
...
![Predikce pro různé n_context — 100-epoch model](figures/fig_big_06b_context_size_grid.png)
...
![MSE vs n_context — random-HP model](figures/fig_rand_05_context_size_mse.png)
...
![Predikce pro různé n_context — random-HP model](figures/fig_rand_05b_context_size_grid.png)
...
> Pro každou realizaci: vygenerovat GP($\ell$, noise=0.01) sekvenci délky 60, prvních 30 jako train, zbytek jako test. `MSE` = `mean((pfn_mean − gp_mean)²)`. `Medián` robustnější vůči outlierům než průměr.
...
![MSE napříč lengthscale hodnotami](figures/fig_rand_06_ls_accuracy.png)
...
![Predikce pro různé lengthscale hodnoty](figures/fig_rand_06b_ls_predictions.png)
...
![HP identifikace — LS=0.1](figures/fig_rand_07a_hp_ident_ls01.png)
...
![HP identifikace — LS=0.2](figures/fig_rand_07b_hp_ident_ls02.png)
...
![PFN vs ML-II](figures/fig_rand_08_pfn_vs_ml2.png)
...
> `Marginalizace` = průměr GP predikcí přes 30 rovnoměrně rozložených hodnot $\ell \in [0.05, 1.0]$ s uniformními váhami — `mean([gp_predict(ls=l) for l in ls_grid])`. Reference `True GP` = GP se správným $\ell=0.3$.
...
![Marginalizace vs PFN vs ML-II](figures/fig_rand_09_marginalization.png)
...
![Detail attention hlavy L0H0](figures/fig_big_07a_head_detail_L0H0.png)
*Obrázek 20: Analýza hlavy — vrstva 0, hlava 0. Korelace s RBF = 0.46 (nejvyšší v této vrstvě).*

![Detail attention hlavy L5H3](figures/fig_big_07e_head_detail_L5H3.png)
*Obrázek 21: Analýza hlavy — vrstva 5, hlava 3. Nejlepší jednotlivá hlava: MSE(Head, GP) = 0.1326, stále 12 000× horší než celý model.*

![Scan všech 48 hlav — heatmapa](figures/fig_big_07g_head_scan_heatmap.png)
...
*Obrázky extrahované z výstupů notebooků skriptem `background/extract_figures.py` → `background/figures/`.*
